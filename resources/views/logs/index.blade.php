...
@section('description')
    Mencatat semua kegiatan yang dilakukan pada hari ini.
@endsection
                ...
                <tbody>
                    @foreach ($logs as $log)
                        ...
                        <tr>
                            ...
                            <td class="capitalize">{{ $log->aktivitas }}</td>
                            <td>
                                @if (!$filePath)
                            ...
                            @if (Auth::user()->hasRole('admin'))
                                <td>
                                    @if (!empty($log->atasan))
                                        {{ $log->atasan->name }}
                                    @else
                                        <span class="text-gray-500 italic">Tidak ada atasan</span>
                                    @endif
                                </td>
                            @endif
                            <td>
                                <div class="flex items-center gap-2">
                                    <button data-id="{{ $log->id }}" data-jenis="edit"
                                    ...
                                    <button data-id="{{ $log->id }}" data-jenis="delete"
...
@push('scripts')
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            if (document.getElementById("logs-table") && typeof simpleDatatables.DataTable !== 'undefined') {
                const dataTable = new simpleDatatables.DataTable("#logs-table", {
                    searchable: true,
                });
            }
            ...
            buttonAdd.addEventListener('click', function() {
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        const picker = flatpickr("#tanggal", {
            ...
            $('#logs-table').on('click', '.action', function() {
                let data = $(this).data();
                let id = data.id || null;
