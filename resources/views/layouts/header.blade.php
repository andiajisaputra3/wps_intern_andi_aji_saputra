...
<header x-data="{ menuToggle: false }"
    class="sticky top-0 flex w-full bg-white border-gray-200 z-20 dark:border-gray-800 dark:bg-gray-900 lg:border-b">
    <div class="flex flex-col items-center justify-between flex-grow lg:flex-row lg:px-6">
        <div
            ...
            <button
                ...
                class="flex items-center justify-center w-10 h-10 text-gray-500 border-gray-200 rounded-lg z-20 dark:border-gray-800 dark:text-gray-400 lg:h-11 lg:w-11 lg:border"
            ...
            <a href="{{ route('dashboard.index') }}" class="lg:hidden flex items-center gap-x-4">
                <div>
        ...
        <div :class="menuToggle ? 'flex' : 'hidden'"
            ...
            <div class="relative" x-data="{ dropdownOpen: false }" @click.outside="dropdownOpen = false">
                <a href="#" @click.prevent="dropdownOpen = ! dropdownOpen"
                    class="flex items-center text-gray-700 dark:text-gray-400 space-x-3">
                    {{-- Avatar & Inisial --}}
                    <div class="h-11 w-11 rounded-full overflow-hidden flex items-center justify-center">
                        <img id="profileImage"
                            src="{{ $user->profile_photo_path ? asset('storage/' . $user->profile_photo_path) : '' }}"
                            alt="Avatar"
                            class="{{ $user->profile_photo_path ? '' : 'hidden' }} w-full h-full object-cover">

                        <div id="initialsAvatar"
                            class="{{ $user->profile_photo_path ? 'hidden' : '' }} w-11 h-11 flex items-center justify-center rounded-full bg-gray-200 text-gray-600 hover:bg-gray-300 dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-gray-300 font-bold text-sm transition-colors"
                            data-name="{{ $user->name }}"></div>
                    </div>

                    {{-- Nama User --}}
                    <span id="profileName" class="font-medium text-theme-sm">
                        {{ $user->name }}
                    </span>

                    {{-- Ikon panah --}}
                    <svg :class="dropdownOpen && 'rotate-180'"
                        class="stroke-gray-500 dark:stroke-gray-400 transition-transform duration-200" width="18"
                        height="20" viewBox="0 0 18 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M4.3125 8.65625L9 13.3437L13.6875 8.65625" stroke-width="1.5" stroke-linecap="round"
